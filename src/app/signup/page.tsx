...
import { ArrowLeft, Eye, EyeOff, CheckCircle2, Sparkles } from "lucide-react";
...
export default function SignUpPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      alert("Account created successfully! Check your inbox to verify your email.");
    }, 800);
  };
      ...
      <div className="lg:w-1/2 bg-[#0B191E] p-8 lg:p-14 text-white flex flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative z-10 my-12 flex flex-col items-center text-center">
          ...
          <h2 className="text-2xl sm:text-3xl font-extrabold max-w-md">
            Start finding your best leads in seconds.
          </h2>
          <p className="text-slate-400 text-xs sm:text-sm max-w-sm mt-2 leading-relaxed">
            No credit card required. Receive 50 complimentary ICP-qualified leads immediately upon sign-up.
          </p>

          <div className="mt-8 flex items-center gap-5 text-xs text-slate-300">
            <span className="flex items-center gap-1.5">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" /> Free Trial
            </span>
            <span className="flex items-center gap-1.5">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" /> No Card Needed
            </span>
            <span className="flex items-center gap-1.5">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" /> Instant Setup
            </span>
          </div>
        </div>
        ...
        <div className="w-full max-w-md bg-white rounded-3xl p-8 sm:p-10 border border-slate-200/90 shadow-xl shadow-slate-200/50">
          ...
          <div className="mb-6">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Start finding, qualifying, and enriching leads with autonomous AI agents.
            </p>
          </div>
          ...
          {/* Google SSO Button ONLY - Explicitly excluding Microsoft as requested */}
          <button
            type="button"
